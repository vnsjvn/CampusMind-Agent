...
完整聊天记录写入 MySQL 的 `chat_sessions`、`chat_messages` 等表；压缩后的长期会话记忆写入 `long_term_memories`，并记录来源消息数和版本。Redis 保存每个会话最近 `REDIS_MEMORY_MAX_MESSAGES` 条短期上下文以及长期摘要缓存，并通过 `REDIS_MEMORY_TTL_SECONDS` 自动过期。Redis 不可用时默认回退 MySQL；生产环境可设置 `REDIS_MEMORY_REQUIRED=true` 让服务快速失败，避免配置存在但实际未使用。
...
项目包含两类互补评测：60条固定集用于Recall@K、MRR等确定性检索回归；12条人工参考集使用RAGAS 0.4.3评估Context Precision、Context Recall、Faithfulness和Answer Relevancy。RAGAS使用独立虚拟环境，避免其LangChain依赖影响生产Runtime。完整配置、命令和指标口径见 `docs/evaluation.md`。
